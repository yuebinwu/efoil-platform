      ...
      <table className="w-full bg-white rounded-2xl shadow-sm border border-gray-100">
        <thead>
          <tr>
            ...
            <th className="p-4">提交時間</th>
            <th className="p-4">受理時間</th>
            <th className="p-4">完成時間</th>
            <th className="p-4">確認受理</th>
            <th className="p-4">填寫報告</th>
            <th className="p-4">打印報告</th> {/* 新增操作欄 */}
          </tr>
        </thead>
            ...
            repairs.map((r) => (
              <tr key={r.id}>
                ...
                <td className="p-4">
                  {/* 這裡是您要的打印按鈕 */}
<button 
  onClick={() => window.open(`/print-center?id=${r.id}&type=repair`, '_blank')}
  className="..."
>
  打印報告
</button>
