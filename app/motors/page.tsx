...
import ProductImage from '@/components/ProductImage'; // 引入修复闪烁的组件
...
export default async function MotorsPage() {
  ...
  if (error || !allItems) {
    return <div className="p-10 text-center text-red-500">Failed to load motors</div>;
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-8">Motors</h1>
          ...
          return (
            <div key={item.id} className="border border-gray-200 p-4 rounded-xl shadow-sm hover:shadow-md transition-shadow flex flex-col bg-white">
              <div className="relative w-full h-48 mb-4 overflow-hidden rounded-lg">
                {/* 使用统一的 ProductImage 组件，解决图片加载导致的闪动 */}
